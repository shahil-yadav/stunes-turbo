...
function useGetActiveStateFromPath() {
  ...
  const state = pathname.split("/").length === 4 ? pathname.split("/").at(-1) : "all"

  const parsedResults = stateSchema.safeParse(state)
  if (!parsedResults.success) throw new Error("Zod states are not in sync with search bar")
...
export function Search() {
  ...
  return isSearchEnabled && <TriggerSearchBar />
}
...
function ActiveInactiveLink({ actve = false, text }: { actve: boolean; text: string }) {
